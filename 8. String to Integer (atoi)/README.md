...
3. **Conversion**: Read the integer by skipping leading zeros until a non-digit character is encounter or the end of the string is reached. If no digits were read, then the result is `0`
...
Explanation: The starred (*) characters are what is read in and the caret is the current reader position
...
Expanation:
> Step 1: "0-1" (no characters are read because there is no leading whitespace)
           ^
...
Expanation: Reading stops at the first non-digit character 'w'
```
